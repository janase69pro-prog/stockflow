          ...
          return (
            <div key={p.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <form action={withdrawItem.bind(null, p.id)}><Button variant="outline" className="w-full" disabled={p.current_stock < 1 || !canAfford}>Retirar</Button></form>
      ...
      {sellingItem && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm space-y-4">
            ...
            <form action={async (fd) => { await sellItem(sellingItem.id, parseFloat(fd.get('price') as string)); setSellingItem(null) }}>
      ...
      {transferringItem && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm space-y-4">
            ...
            <form action={async (fd) => { await transferItem(transferringItem.id, fd.get('targetId') as string); setTransferringItem(null) }}>
